...
from dtcwt.tf.lowlevel import _HAVE_TF as HAVE_TF, rowifilt
...
def test_odd_filter():
    with raises(ValueError):
        rowifilt(mandrill_t, (-1,2,-1), (-1,2,1))


@skip_if_no_tf
def test_different_size_h():
    with raises(ValueError):
        rowifilt(mandrill_t, (-1,2,1), (-0.5,-1,2,-1,0.5))


@skip_if_no_tf
def test_zero_input():
    Y = rowifilt(mandrill_t, (-1,1), (1,-1))
...
def test_bad_input_size():
    with raises(ValueError):
        rowifilt(mandrill_t[:,:,:511], (-1,1), (1,-1))
...
def test_output_size():
    Y = rowifilt(mandrill_t, (-1,1), (1,-1))
    assert Y.shape[1:] == (mandrill.shape[0], mandrill.shape[1]*2)
...
def test_output_size_non_mult_4():
    Y = rowifilt(mandrill_t, (-1,0,0,1), (1,0,0,-1))
    assert Y.shape[1:] == (mandrill.shape[0], mandrill.shape[1]*2)
...
def test_equal_numpy_qshift1():
    ...
    ref = np_colifilt(mandrill.T, ha, hb).T
    y_op = rowifilt(mandrill_t, ha, hb)
    with tf.Session() as sess:
        y = sess.run(y_op)
    np.testing.assert_array_almost_equal(y[0], ref, decimal=4)
